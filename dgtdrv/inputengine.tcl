...
#    database upon built.
#
#    To enable it just copy the file to tcl/tools/ subdir of the scid
#    source tree and add it to the TCLS variable within scids Makefile
#    as the last entry.
#
#    $Id: inputengine.tcl,v 1.3 2008/11/15 16:48:46 arwagner Exp $
#    Last change: <Sun, 2008/02/24 16:32:55 arwagner ingata>
#    Author     : Alexander Wagner
#    Language   : TCL
#
#-------------------------------------------------------------------------
#
#    Copyright (C) 2007, 2008 by Alexander Wagner
#
...
#----------------------------------------------------------------------
# Add "Connect Hardware" to Tools Menue
...
pack .button.space4 .button.inputengine \
...
namespace eval inputengine {
	...
	proc readFromEngine {} {
		...
		switch -regexp -- $line \
			{^\[[A-Za-z]} {
				# We got some pgn header field
				logEngine "Header detected"
			} \
			"^move *" {
				logEngine "$line"
			...
			"info *" {
				...
				switch -regexp -- $event \
					...
					"DGT Projects*" {
						# we found a DGT board, display the proper logo
					...
					"string FEN *" {
						...
						# yet, so this signals a new program startupuuuu
						# accordingly.
						set InputEngine(init) 0
					} \
					"FEN *" {
						...
						if { $::inputengine::InputEngine(init) == 0 }  {
							...
							logEngine $fen
							if {$fen != $int} {
								logEngine "Wrong Position"
							} else {
								logEngine "Everything ok"
							}
						}
					} \
					{moving piece: [A-Z] *} {
